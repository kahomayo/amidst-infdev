...
public enum ChunkBasedGen {
    ...
    public static int[] mapChunkBased(int x, int y, int width, int height, boolean useQuarterResolution, ChunkAcessor chunkAcessor) throws MinecraftInterfaceException {
        ...
        for (int chunkZ = chunkZBegin; chunkZ <= chunkZEnd; ++chunkZ) {
            ...
            for (int chunkX = chunkXBegin; chunkX <= chunkXEnd; ++chunkX) {
                int[] biomes = chunkAcessor.getChunk(chunkZ, chunkX);
                // add to result array
